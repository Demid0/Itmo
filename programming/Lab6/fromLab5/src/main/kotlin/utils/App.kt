...
class App: KoinComponent {
    private val clientInvoker: Invoker by inject()
    ...
    fun run(inputReader: BufferedReader, outputWriter: PrintWriter) {
        while (true) {
            ...
            if (commandPacket == null) {
                writer.print("Command not found or wrong args.\n")
                writer.flush()
                continue
            }
            else if (commandPacket.objectArg != null) commandPacket.objectArg = asker.askRoute(reader, writer)
            val output = clientInvoker.invoke(commandPacket)
